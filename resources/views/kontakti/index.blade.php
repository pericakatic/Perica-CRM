...
@section('content')
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Kontakti</h1>
            <p class="text-xs text-gray-500 mt-1">Upravljanje kontaktima i njihovim fazama u CRM-u</p>
        </div>

        <div class="flex items-center gap-3">
            <div class="inline-flex rounded-lg bg-gray-100 p-1 border border-gray-200">
                <a href="{{ route('kontakti.index', ['view' => 'kanban']) }}"
                ...
                <a href="{{ route('kontakti.index', ['view' => 'table']) }}"
    ...
    @if(request('view', 'kanban') === 'kanban')
        ...
        @php
            $statusi = [
                'lead'        => ['naziv' => 'Lead', 'bg' => 'bg-amber-50', 'text' => 'text-amber-700', 'border' => 'border-amber-200'],
                'kontaktiran' => ['naziv' => 'Kontaktiran', 'bg' => 'bg-blue-50', 'text' => 'text-blue-700', 'border' => 'border-blue-200'],
                'kupac'       => ['naziv' => 'Kupac', 'bg' => 'bg-emerald-50', 'text' => 'text-emerald-700', 'border' => 'border-emerald-200'],
            ];

            $grupisaniKontakti = $kontakti->groupBy(fn($item) => strtolower($item->status));
        @endphp

        <div class="grid grid-cols-1 md:grid-cols-3 gap-5 items-start">
            @foreach($statusi as $statusKljuc => $info)
                @php
                    $kontaktiUStatusu = $grupisaniKontakti->get($statusKljuc, collect());
                @endphp

                <div class="bg-gray-100/80 rounded-xl p-4 border border-gray-200/80 flex flex-col max-h-[calc(100vh-220px)]">
                    <div class="flex items-center justify-between mb-3 px-1">
                        <div class="flex items-center gap-2">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold {{ $info['bg'] }} {{ $info['text'] }} border {{ $info['border'] }}">
                                {{ $info['naziv'] }}
                            </span>
                            <span class="text-xs font-medium text-gray-500">({{ $kontaktiUStatusu->count() }})</span>
                        </div>
                    </div>

                    <div class="space-y-3 overflow-y-auto pr-1">
                        @forelse($kontaktiUStatusu as $kontakt)
                            <div class="bg-white rounded-lg p-4 shadow-2xs border border-gray-200 hover:border-gray-300 transition-all">
                                <!-- Ime i Prezime -->
                                <div class="flex justify-between items-start mb-2">
                                    <h3 class="font-semibold text-gray-900 text-sm">{{ $kontakt->ime }} {{ $kontakt->prezime }}</h3>
                                </div>

                                @if(optional($kontakt->tvrtka)->naziv)
                                    <div class="flex items-center gap-1.5 text-xs font-medium text-indigo-600 mb-2">
                                        <svg class="w-3.5 h-3.5 text-indigo-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5m0 0h4m-4 0V11m0 4h4m-4-8h4m-4-4h4"/>
                                        </svg>
                                        <span class="truncate">{{ $kontakt->tvrtka->naziv }}</span>
                                    </div>
                                @endif
                                ...
                                <div class="space-y-1 text-xs text-gray-500 mb-4">
                                    @if($kontakt->email)
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            <span class="truncate">{{ $kontakt->email }}</span>
                                        </div>
                                    @endif
                                ...
                                <div class="flex items-center justify-end gap-2 pt-2 border-t border-gray-100">
                                    <a href="{{ route('kontakti.edit', $kontakt->id) }}"
                                    ...
                                    <form action="{{ route('kontakti.destroy', $kontakt->id) }}" method="POST"
                                          onsubmit="return confirm('Sigurno želite obrisati ovaj kontakt?');" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                        ...
                        @empty
                            <div class="p-4 text-center border-2 border-dashed border-gray-200 rounded-lg">
                                <p class="text-xs text-gray-400">Nema kontakata u ovom statusu.</p>
                            </div>
                        @endforelse
                    </div>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-2xs border border-gray-200 overflow-hidden">
            <table class="w-full text-left border-collapse">
                ...
                <tr class="bg-gray-50/80 border-b border-gray-200 text-gray-600 text-xs uppercase tracking-wider font-semibold">
                    <th class="p-3.5">Ime i Prezime</th>
                    <th class="p-3.5">Tvrtka</th>
                    <th class="p-3.5">Email</th>
                    <th class="p-3.5">Telefon</th>
                    <th class="p-3.5">Status</th>
                    <th class="p-3.5 text-right">Akcija</th>
                </tr>
                </thead>
                <tbody class="divide-y divide-gray-200 text-sm">
                @forelse($kontakti as $kontakt)
                    <tr class="hover:bg-gray-50/60 transition-colors">
                        <td class="p-3.5 font-medium text-gray-900">{{ $kontakt->ime }} {{ $kontakt->prezime }}</td>
                        <td class="p-3.5 text-gray-600">{{ $kontakt->tvrtka->naziv ?? '-' }}</td>
                        <td class="p-3.5 text-gray-600">{{ $kontakt->email ?? '-' }}</td>
                        <td class="p-3.5 text-gray-600">{{ $kontakt->telefon ?? '-' }}</td>
                        <td class="p-3.5">
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-700 border border-gray-200 capitalize">
                                {{ $kontakt->status }}
                            </span>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="p-6 text-center text-gray-500">Nema unesenih kontakata.</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
